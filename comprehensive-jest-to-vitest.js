...
const REPLACEMENTS = [
  ...
  { from: /jest\.advanceTimersToNextTimer\(/g, to: 'vi.advanceTimersToNextTimer(' },
...
async function findAllFiles(dir, excludeDirs = ['node_modules', '.git', 'dist', 'build', 'coverage']) {
  const files = [];
    ...
    await Promise.all(
      entries.map(async (entry) => {
        ...
        if (entry.isDirectory()) {
          ...
          if (fullPath.match(/\.(js|jsx|ts|tsx|json|md|yaml|yml|sh|config\.js|config\.ts)$/)) {
            files.push(fullPath);
          }
        }
      })
  ...
  try {
    ...
    if (filePath.match(/\.(test|spec)\.(js|jsx|ts|tsx)$/)) {
      if (!content.includes('import { vi') && !content.includes('const { vi') && content.match(/jest\./)) {
        const hasExistingVitestImport = content.includes('from "vitest"') || content.includes("from 'vitest'");
...
async function main() {
  ...
  console.log('   3. Remove Jest packages: npm uninstall jest @types/jest ts-jest babel-jest');
