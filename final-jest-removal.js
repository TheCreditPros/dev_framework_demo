...
const REPLACEMENTS = [
  ...
  { from: /"testEnvironment":\s*"jest-environment-jsdom"/g, to: '"testEnvironment": "jsdom"' },
  { from: /"testEnvironment":\s*"jest-environment-node"/g, to: '"testEnvironment": "node"' },
...
async function findAllFiles(dir, excludeDirs = ['node_modules', '.git', 'dist', 'build', 'coverage', '.next', 'out', '.cache']) {
  const files = [];
      ...
      await Promise.all(
        entries.map(async (entry) => {
          ...
          if (entry.isDirectory()) {
            ...
            if (!excludeDirs.includes(entry.name) && (entry.name === '.husky' || !entry.name.startsWith('.'))) {
...
async function processFile(filePath) {
  try {
    ...
    if (filePath.includes('jest-to-vitest') || filePath.includes('jest-conversion')) {
        ...
        if (!hasViImport) {
          if (hasVitestImport) {
            ...
            for (let i = 0; i < lines.length; i++) {
              if (lines[i].match(/^import\s+/) || lines[i].match(/^const\s+.*=\s*require/)) {
                lastImportIndex = i;
              }
            }
    ...
    if (path.basename(filePath) === 'package.json') {
      try {
        ...
        const jestPackages = [
          ...
          'jest-circus', 'jest-watch-typeahead', 'jest-resolve',
          'jest-runner', 'jest-cli', 'jest-config'
        ];
    ...
    if (filePath.match(/\.(yml|yaml)$/)) {
      ...
      content = content.replace(/npm\s+test\s+--\s+jest/g, 'npm test -- vitest');
...
async function verifyNoJestReferences(rootDir) {
  ...
  for (const file of testFiles) {
    ...
    if (file.includes('node_modules') || file.includes('jest-to-vitest') || file.includes('jest-conversion') || file.includes('jest-removal')) {
      continue;
    }
...
async function main() {
  ...
  console.log('   3. Commit changes: git add -A && git commit -m "chore: complete vitest to Vitest migration"');
  console.log('   4. Update CI/CD pipelines if needed');
  console.log('   5. Update documentation to reference Vitest instead of vitest');
