...
describe('APIProcessAnalyzer', () => {
  ...
  beforeEach(async () => {
    ...
    fs.existsSync.mockImplementation((filePath) => {
      ...
      if (filePath === path.join(mockProjectRoot, 'reports', 'api-processes'))
        return false;
...
`;
    ...
    const { default: APIProcessAnalyzer } = await import(
      '../../scripts-complex/api-process-analyzer.js'
    );
  ...
  describe('identifySequentialOperations', () => {
    ...
    it('should group operations by common tags', () => {
      ...
      const processes = analyzer.identifySequentialOperations(spec);
      const userManagementProcess = processes.find(
        (p) => p.name === 'User Management'
      );

      expect(userManagementProcess).toBeDefined();
      expect(
        userManagementProcess.operations.map((op) => op.operationId)
      ).toEqual([
        'createUser',
        'getUsers',
        'getUser',
...
`
  ...
  describe('validateProcessDocumentation', () => {
    it('should validate well-documented processes', () => {
      ...
      const issues = analyzer.validateProcessDocumentation(
        processes,
        '/mock/project/openapi.yaml'
      );
...
`
      ...
      expect(issues.some((issue) => issue.type === 'step-description')).toBe(
        true
      );
    });
    ...
    it('should validate multi-step process documentation quality', () => {
      ...
      const documentationQuality = analyzer.assessProcessDocumentationQuality(
        userProcess.operations
      );
      ...
      const processes = [
        {
          ...
          operations: [{ operation: { operationId: 'getCreditReport' } }],
        },
      ];
      ...
      const processes = [
        {
          ...
          tag: 'credit-data',
          operations: [{ operation: { operationId: 'accessCreditData' } }],
        },
      ];
      ...
      const processes = [
        {
          ...
          tag: 'credit-pull',
          operations: [{ operation: { operationId: 'pullCreditReport' } }],
        },
      ];
  ...
  describe('generateProcessFlowDocumentation', () => {
    it('should generate comprehensive process documentation', () => {
      ...
      const documentation = analyzer.generateProcessFlowDocumentation(
        processes,
        '/mock/project/openapi.yaml'
      );
  ...
  describe('provideImprovementRecommendations', () => {
    it('should provide actionable process improvement recommendations', () => {
      ...
      const recommendations = analyzer.provideImprovementRecommendations(
        issues,
        processes
      );
    ...
    it('should recommend process simplification for complex workflows', () => {
      ...
      expect(
        recommendations.some((rec) => rec.title.includes('Simplification'))
      ).toBe(true);
    });
  ...
  describe('Integration Tests', () => {
    it('should analyze complete API processes workflow', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('API Process Analysis Complete')
      );
...
`
      ...
      const creditDisputeProcess = processes.find((p) =>
        p.tag.includes('dispute')
      );
